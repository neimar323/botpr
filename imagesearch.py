...
'''
def imagesearch(image, precision=0.8, save=True):
    im = pyautogui.screenshot()
    if save:
        im.save('test.png') #usefull for debugging purposes, this will save the captured region as "testarea.png"
    img_rgb = np.array(im)
    img_gray = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)
    template = cv2.imread(image, 0)
    template.shape[::-1]
...
'''
def imagesearch_numLoop(image, timesample, maxSamples, precision=0.8, save=True):
    pos = imagesearch(image, precision,save)
    count = 0
    while pos is None:
        print(image+" not found, waiting")
        time.sleep(timesample)
...
def imgClick(image,timesample, maxSamples, precision=0.8):
    pos = imagesearch_numLoop(image, timesample, maxSamples, precision)
    if pos is not None:
        click_image(image,pos, 'left', 0.1, 0.1)
        return True
    else:
        return False